...
class TestQuery:
    ...
    Attributes:
        ...
        type: 问题类型 (detail/list/general) - 由系统检测，不预定义
        filters: 过滤条件字典 - 由系统在运行时提取，不预定义
    """
...
class TestDataManager:
    ...
    def save_test_queries(self, queries: List[TestQuery], filename: str) -> Path:
        """
        保存测试数据到JSON文件
    ...
    def generate_from_data(
        self, data_path: str, sample_size: int = 20, output_file: Optional[str] = None
    ) -> List[TestQuery]:
        """
    ...
    def _generate_queries(
        self, recipes: List[Dict[str, Any]], sample_size: int
    ) -> List[TestQuery]:
        """
        从菜谱列表生成测试query

        只生成 text 和 expected_docs，query type 和 filters 由系统在运行时检测。
        ...
        existing_names = [q.expected_docs[0] for q in queries if q.expected_docs]
        while len([q for q in queries if q.text.endswith("怎么做")]) < detail_count:
        ...
        for _ in range(list_count):
            ...
            queries.append(TestQuery(id=f"q{query_id}", text=f"有哪些{cat}推荐"))
            query_id += 1
